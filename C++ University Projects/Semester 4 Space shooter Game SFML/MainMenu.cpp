...
{	
	...
	Game_Title.setString("Space Shooter");
	Game_Title.setPosition(s_Content->s_Window->getSize().x / 2 - 0.5 * Game_Title.getGlobalBounds().width, s_Content->s_Window->getSize().y / 2 - 100.f);
	...
	menu[0].setPosition(s_Content->s_Window->getSize().x / 2 - 0.5 * menu[0].getGlobalBounds().width, s_Content->s_Window->getSize().y / 2);
	...
	menu[1].setPosition(s_Content->s_Window->getSize().x / 2 - (0.5 * menu[1].getGlobalBounds().width), s_Content->s_Window->getSize().y / 2 + 25.f);
	...
	menu[2].setPosition(s_Content->s_Window->getSize().x / 2 - 0.5 * menu[2].getGlobalBounds().width, s_Content->s_Window->getSize().y / 2 + 50.f);
}
...
void MainMenu::Update()
{
	...
	{
		s_Content->s_States->PutOnStack(std::make_unique<Nick>(s_Content), true);
	}
	else if (LeaderBoards_Button_Pressed)
	{
		s_Content->s_States->PutOnStack(std::make_unique<Leaderboards>(s_Content), true);
	}
